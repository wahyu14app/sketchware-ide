...
class DependencyResolver(
    ...
    private val buildSettings: Any
) {
    private fun getBuildSettingsValue(key: String, defaultValue: String): String {
        return try {
            val method = buildSettings.javaClass.getMethod("getValue", String::class.java, String::class.java)
            method.invoke(buildSettings, key, defaultValue) as String
        } catch (e: Exception) {
            defaultValue
        }
    }

    private val extractedCompileAssetsPath: File by lazy {
        try {
            val clazz = Class.forName("mod.jbk.build.BuiltInLibraries")
            clazz.getField("EXTRACTED_COMPILE_ASSETS_PATH").get(null) as File
        } catch (e: Exception) {
            File("/sdcard/.sketchware/data/system")
        }
    }
    ...
    fun resolveDependency(callback: DependencyResolverCallback) = runBlocking {
        ...
        val libraryJars = listOf(
            extractedCompileAssetsPath.toPath().resolve("core-lambda-stubs.jar"),
            Paths.get(
                getBuildSettingsValue(
                    "android_jar_path",
                    File(extractedCompileAssetsPath, "android.jar").absolutePath
                )
            )
        )
        val dependencyClasspath = mutableListOf<Path>()

        val classpath = getBuildSettingsValue("classpath", "")
