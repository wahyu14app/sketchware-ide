...
class KotlinCompiler(
    ...
    fun compile() {
        ...
        val javaVersion = builder.build_settings.getValue(
            mod.hey.studios.build.BuildSettings.SETTING_JAVA_VERSION,
            mod.hey.studios.build.BuildSettings.SETTING_JAVA_VERSION_1_8
        )
        ...
        val args = K2JVMCompilerArguments().apply {
            ...
            noReflect = true
            noStdlib = true
            jvmTarget = if (javaVersion == "1.7") "1.8" else javaVersion
    ...
    private fun areWarningsEnabled(): Boolean {
        return try {
            val buildSettingsField = builder.javaClass.getField("build_settings")
            val buildSettings = buildSettingsField.get(builder)
            val getValueMethod = buildSettings.javaClass.getMethod("getValue", String::class.java, String::class.java)
            val value = getValueMethod.invoke(buildSettings, "no_warnings", "true") as String
            value != "true"
        } catch (e: Exception) {
            false
        }
